...
|----------|------|-------------|
| Risk predictions CSV | `reports/local/p4_hybrid_spike_gate/best_ml_gate_risk_predictions.csv` | 2880 rows, ML gate probabilities replacing `high_spike_prob` |
| Manifest | `reports/local/p4_hybrid_spike_gate/best_ml_gate_manifest.json` | Metadata, metrics, known limitations |
| Evaluation script | `scripts/evaluate_p4_hybrid_spike_gate.py` | Full evaluation pipeline (can regenerate) |
...
| `timestamp` | str | Full timestamp (YYYY-MM-DD HH:MM:SS) |
| `high_spike_prob` | float | ML gate probability (0-1) — **drop-in replacement** for old risk model |
| `risk_source` | str | `"ml_gate"` |
| `gate_type` | str | `"ml_gate"` |

### Recommended W4 Pipeline

```
risk_predictions_ml_gate.csv
       │
       ▼
run_correction(prediction_pack, risk_predictions_ml_gate, profile="aggressive")
       │
       ▼
final_pred = guardrail.evaluate(...).final_pred
```

W4 must:
1. Replace `high_spike_prob` column with values from `best_ml_gate_risk_predictions.csv`
2. Use `aggressive` correction profile (threshold 0.40 — aligns with ML gate calibration)
3. Preserve all other pipeline components (`residual_lift.py`, `guardrail.py`) unchanged
4. Do NOT use rule_gate probabilities — insufficient recall for production

### Expected Metrics (reproduced on canonical pack)

| Metric | Value | Target |
|--------|-------|--------|
| sMAPE (floor50) | 22.43 | — (base model constrained) |
| Severe underestimates | **56** | ≤ 63 ✅ |
| False lift rate | **9.06%** | ≤ 10% ✅ |
| Spike recall | 0.80 | — |
| Precision | 0.50 | — |
