...
# The original_pred-from-run-output invariant requires the module to build
# features from the realtime ledger. Without it (e.g. a checkout that has no
# outputs/ledger), the shadow is fully degraded and original_pred is a
# placeholder, so this specific assertion is skipped rather than failing.
HAS_REAL_LEDGER = Path(REAL_LEDGER).exists()
...
@pytest.mark.skipif(not HAS_REAL_LEDGER, reason="requires realtime ledger (outputs/ledger) to exercise run-output original_pred path")
