...
from fusion.meta_learner_v2 import fit_meta_learners_from_long_table, apply_meta_learners
...
def run_learner_stage(args):
    ...
    for target in targets:
        ...
        # Use improved meta learner
        models, report_df = fit_meta_learners_from_long_table(
            contract_df,
            n_estimators=100,
            max_depth=3,
            learning_rate=0.1,
        ...
        joblib.dump(models, meta_learner_path)
...
def run_fuse_stage(args):
    ...
    for target in targets:
        ...
        models = joblib.load(meta_learner_path)
        
        # Prepare forecast data in contract format
        forecast_long = _build_forecast_long_with_truth(forecast_df, target=target)
        
        # Apply meta learner
        fused = apply_meta_learners(
            forecast_long,
            models,
            task=target,
            test_start=run_date,
