...
def fit_meta_learners_from_long_table(
    ...
    *,
    ...
    for (task, period), group in wide.groupby(["task", "period"], sort=True):
        ...
        coef_map = dict(zip(segment_model.model_names, estimator.feature_importances_.tolist()))
        report_rows.append(
            {
                "task": task,
                "period": period,
                ...
                **{f"importance_{name}": float(coef_map.get(name, 0.0)) for name in segment_model.model_names},
            }
        )
